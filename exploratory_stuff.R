...
#dig into char features
str(lapply(train_char, unique), vec.len = 4)

#It looks like NA is represented in character columns by -1 or [] or blank values, 
# lets convert these to explicit NAs. Not entirely sure this is the right thing to do as there 
# are real NA values, as well as -1 values already existing, however it can be tested
...
#Now lets check the cumulative distribution of correlation within these 100 sampled numerical columns. 
